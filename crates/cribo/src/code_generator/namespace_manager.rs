...
/// Create required namespaces for modules that need them.
///
/// This function identifies which modules need namespace objects and creates them
/// upfront to avoid forward reference errors. It handles:
/// - Modules imported as namespaces (import module)
/// - Directly imported modules with exports
/// - Submodules that need namespaces for parent imports
pub fn create_required_namespaces(
    ...
    let directly_imported_modules =
        bundler.find_directly_imported_modules(modules, entry_module_name);

    debug!("Creating namespace objects for modules that need them");
    debug!(
        "namespace_imported_modules: {:?}",
        bundler.namespace_imported_modules
    );
    debug!("directly_imported_modules: {directly_imported_modules:?}");

    // Check each module to see if it needs a namespace
    for (module_name, _, _, _) in modules {
        // Skip the entry module
        if module_name == entry_module_name {
            continue;
        }

        // Check if module has exports
        let has_exports = bundler
            .module_exports
            .get(module_name)
            .is_some_and(Option::is_some);

        // Check if this is a submodule that needs a namespace
        let needs_namespace_for_submodule = bundler.submodule_needs_namespace(module_name);
        ...
        // 3. It's a submodule that's imported by its parent module
        let needs_namespace = bundler.namespace_imported_modules.contains_key(module_name)
        ...
        if needs_namespace {
            ...
            if !bundler.created_namespaces.contains(&namespace_var) {
                debug!("Creating namespace for module '{module_name}'");

                // For now, create an empty namespace that will be populated later
                ...
                bundler.created_namespaces.insert(namespace_var);
                bundler.required_namespaces.insert(module_name.to_string());
            }
        }
    }
