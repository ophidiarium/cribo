            ...
            if let Stmt::Assign(assign) = &stmt {
                ...
                if assign.targets.len() == 1 {
                    if let Expr::Name(target) = &assign.targets[0]
                        && let Expr::Call(call) = assign.value.as_ref()
                    {
                        // Check if this is a namespace creation: _cribo.types.SimpleNamespace()
                        // Only synthetic nodes (generated by cribo) should create namespaces
                        // User code with types.SimpleNamespace gets transformed to use _cribo proxy
                        if ast_builder::is_synthetic_node(call) {
                            let is_namespace_creation = match call.func.as_ref() {
                                // Check for _cribo.types.SimpleNamespace
                                Expr::Attribute(attr)
                                    if attr.attr.as_str() == "SimpleNamespace" =>
                                {
                                    match attr.value.as_ref() {
                                        // _cribo.types
                                        Expr::Attribute(inner_attr)
                                            if inner_attr.attr.as_str() == "types" =>
                                        {
