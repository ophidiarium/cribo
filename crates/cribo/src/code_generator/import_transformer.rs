...
impl<'a> RecursiveImportTransformer<'a> {
    ...
    fn transform_stmt(&mut self, stmt: &mut Stmt) {
        ...
        let is_hoisted = if is_import {
            self.bundler.is_hoisted_import(stmt)
        } else {
            false
        };
        ...
        if needs_transformation {
            // Transform the import statement
            let transformed = self.transform_statement(stmt);

            log::debug!(
                "transform_stmt: Transforming import in module '{}', got {} statements back",
                self.module_name,
                transformed.len()
            );
            ...
            match stmt {
                ...
                Stmt::ClassDef(class_def) => {
                    ...
                    if let Some(ref mut arguments) = class_def.arguments {
                        for base in &mut arguments.args {
                            if has_hard_deps {
                                // For classes with hard dependencies, check if this base is a hard
                                // dep
                                let base_str = if let Expr::Name(name) = base {
                                    name.id.as_str().to_string()
                                } else if let Expr::Attribute(attr) = base {
                                ...
                                if !is_hard_dep_base {
                                    ...
                                    log::debug!(
                                        "Skipping transformation of hard dependency base class \
                                         {base_str} for class {class_name}"
                                    );
                                }
                            } else {
                                // No hard dependencies, transform normally
