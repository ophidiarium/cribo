...
impl<'a> ProcessingPhase<'a> {
    ...
    pub fn execute(
        ...
        let dep_analysis = self.analyze_wrapper_dependencies(classification, modules);
    ...
    fn analyze_wrapper_dependencies(
        ...
        for (module_id, _, _, _) in &classification.wrapper_modules {
            self.bundler.wrapper_modules.insert(*module_id);
        }
        ...
        let has_circular_wrapped_modules = classification
            .wrapper_modules
            .iter()
            .any(|(module_id, _, _, _)| self.bundler.is_module_in_circular_deps(*module_id));
    ...
    fn build_circular_groups(&self, params: &BundleParams<'a>) -> CircularGroupContext {
    ...
    fn log_processing_order(&self, params: &BundleParams<'a>) {
    ...
    fn process_circular_group(
        ...
        let params = self.bundler.graph.expect("graph must be set");
        ...
        for (mid, mname) in &members {
            ...
            if !self.bundler.module_synthetic_names.contains_key(mid) {
                ...
                self.bundler.inlined_modules.shift_remove(mid);
            }
            ...
            if !self.bundler.created_namespaces.contains(&sanitized) {
                let synthetic = self
                    .bundler
                ...
                self.bundler.created_namespaces.insert(sanitized.clone());
            }

            self.bundler
                .create_namespace_chain_for_module(mname, &sanitized, all_inlined_stmts);
        ...
        for (mid, mname) in &members {
            ...
            let transform_ctx = ModuleTransformContext {
                ...
                semantic_bundler: self.bundler.semantic_bundler,
                python_version,
            ...
            let init_function =
                InitFunctionBuilder::new(self.bundler, &transform_ctx, symbol_renames)
                    .build(ast.clone())
    ...
    fn process_inlinable_module(
        ...
        if module_name != self.bundler.entry_module_name {
            ...
            if !self.bundler.created_namespaces.contains(&namespace_var) {
                ...
                let mut keywords = vec![Keyword {
                    node_index: self.bundler.create_node_index(),
                ...
                self.bundler
                    .created_namespaces
                    .insert(namespace_var.clone());

                self.bundler.create_namespace_chain_for_module(
                    module_name,
                    &namespace_var,
                    all_inlined_stmts,
                );
        ...
        self.bundler
            .inline_module(module_name, ast, path, &mut inline_ctx);
                ...
                .contains(&current_module_id)
        {
            ...
            let mut population_ctx =
                crate::code_generator::namespace_manager::NamespacePopulationContext {
                    bundled_modules: &self.bundler.bundled_modules,
                    inlined_modules: &self.bundler.inlined_modules,
                    module_exports: &self.bundler.module_exports,
                    tree_shaking_keep_symbols: &self.bundler.tree_shaking_keep_symbols,
                    modules_with_accessed_all: &self.bundler.modules_with_accessed_all,
                    wrapper_modules: &self.bundler.wrapper_modules,
                    module_asts: &self.bundler.module_asts,
                    modules_with_explicit_all: &self.bundler.modules_with_explicit_all,
                    global_deferred_imports: &FxIndexMap::default(),
                    module_init_functions: &self.bundler.module_init_functions,
                    resolver: self.bundler.resolver,
                };
    ...
    fn process_wrapper_module(
        ...
        let is_in_circular = self.bundler.is_module_in_circular_deps(wrapper_module_id);
        ...
        let init_function = InitFunctionBuilder::new(self.bundler, &transform_ctx, symbol_renames)
            .build(ast.clone())
            .expect("Init function transformation should not fail");
        ...
        let namespace_already_exists = self.bundler.created_namespaces.contains(&module_var);

        let mut wrapper_stmts = if namespace_already_exists {
            if self.bundler.emitted_wrapper_inits.insert(wrapper_module_id) {
                crate::ast_builder::module_wrapper::create_init_function_statements(
                    module_name,
                    &init_func_name,
                    init_function,
        ...
        } else if self.bundler.emitted_wrapper_inits.insert(wrapper_module_id) {
        ...
        if !namespace_already_exists {
            self.bundler.created_namespaces.insert(module_var.clone());
        }

        self.bundler
            .create_namespace_chain_for_module(module_name, &module_var, all_inlined_stmts);
    }
}
