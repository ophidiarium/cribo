...
/// Check if an AST node is synthetic (generated by cribo) rather than parsed from source.
///
/// Synthetic nodes are created by the `ast_builder` module and have `TextRange::default()`
/// (0..0) as their range, while parsed nodes have actual text ranges from the source file.
///
/// # Example
/// ```rust
/// use cribo::ast_builder::is_synthetic_node;
///
/// if is_synthetic_node(expr) {
///     // This is generated code
/// } else {
///     // This is from user source
/// }
/// ```
#[inline]
pub fn is_synthetic_node<T: Ranged>(node: &T) -> bool {
    node.range() == TextRange::default()
}
