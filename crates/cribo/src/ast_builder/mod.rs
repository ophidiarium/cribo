...
///
/// Synthetic nodes created by the `ast_builder` module have an empty `TextRange` (len 0),
/// while parsed nodes carry their actual source-file ranges.
///
/// # Example
/// ```rust
...
/// ```
#[inline]
pub(crate) fn is_synthetic_node<T: Ranged>(node: &T) -> bool {
    node.range().is_empty()
}
